...
public partial class customerdashboard : System.Web.UI.Page
{
    ...
    private void SendEmailToCustomer(int nCustomerId, decimal amount, string strTerms, string strPMName, string strCCNum)
    {
        ...
        try
        {
            ...
            string strBCCEmail = "";
            string strBCCEmai2 = ConfigurationManager.AppSettings["CustomerFinalizePaymentBCCEmail"].ToString();
            ...
            if (strToEmail.Length > 4)
            {
                ...
                if ((customeruserinfo)Session["oCustomerUser"] != null)
                {
                    ...
                    //  ProtocolType = csCommonUtility.GetUserInfo(spid).EmailIntegrationType ?? 0;
                }
                email.From = strFrom;
                email.To = strToEmail.Trim();
                email.CC = strCCEmail;
                email.BCC = strBCCEmail;
                email.Subject = "Your payment with " + oCom.company_name;
                email.Body = strTable;
