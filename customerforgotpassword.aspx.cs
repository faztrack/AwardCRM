...
public partial class customerforgotpassword : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["CustForgotPasword"] != null)
            {
                int nCustId = Convert.ToInt32(Session["CustForgotPasword"]);
                hdnCustomerId.Value = nCustId.ToString();

                try
                {
                    DataClassesDataContext _db = new DataClassesDataContext();
                    customeruserinfo objCusUI = new customeruserinfo();
                    objCusUI = _db.customeruserinfos.Single(emp => emp.customerid == Convert.ToInt32(hdnCustomerId.Value));

                    if ((objCusUI.securityquestion ?? "") != "" && (objCusUI.answer ?? "") != "")
                    {
                        lblQuestion.Text = objCusUI.securityquestion;
                    }
                    else
                    {
                        lblQuestion.Text = "<br/>Security answer not set, contact the administrator";
                ...
                Session.Remove("CustForgotPasword");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            if (Convert.ToInt32(hdnCustomerId.Value) > 0)
            {
                ...
                if (txtAnswer.Text.Trim() != objCusUI.answer)
                {
                    lblResult.Text = csCommonUtility.GetSystemErrorMessage("Invalid Answer.");
                    return;
                }
                if (objCusUI.customerusername.Length > 3)
                {
        ...
        else
        {
            if (Convert.ToInt32(hdnCustomerId.Value) > 0)
            {
                ...
                if (objCusUI.customerusername.Length > 3)
                {
                    ...
                    objCusUI.customerpassword = txtPassword.Text.Trim();
                    _db.SubmitChanges();
    ...
    private void SendForgotPassword(user_info objUInfo)
    {
        ...
        msg.From = new MailAddress("contact@faztrack.com");
