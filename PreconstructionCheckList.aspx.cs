...
public partial class PreconstructionCheckList : System.Web.UI.Page
{
    ...
    public void BindSelectedItemGrid()
    {
        ...
        if (hdnCOOrderExist.Value == "1")
        {
            strQ = "select DISTINCT co_pricing_master.location_id AS colId,'LOCATION: '+ location.location_name as colName,ISNULL(sort_id,0) AS sort_id, ISNULL(LocationNoteDetails.LocationNotes,'') AS LocationNotes, ISNULL(LocationNoteDetails.LocationNotesNew,'') AS LocationNotesNew  from co_pricing_master  " +
              ...
              " AND co_pricing_master.estimate_id =" + Convert.ToInt32(hdnEstimateId.Value) + " AND co_pricing_master.customer_id =" + Convert.ToInt32(hdnCustomerId.Value) + " AND is_direct=1 AND co_pricing_master.item_status_id =1 AND co_pricing_master.client_id =" + Convert.ToInt32(ConfigurationManager.AppSettings["client_id"]) + " order by sort_id asc";
        }
        else
        {
            strQ = "select DISTINCT pricing_details.location_id AS colId,'LOCATION: '+ location.location_name as colName,ISNULL(sort_id,0) AS sort_id, ISNULL(LocationNoteDetails.LocationNotes,'') AS LocationNotes, ISNULL(LocationNoteDetails.LocationNotesNew,'') AS LocationNotesNew  from pricing_details  " +
    ...
    protected void grdSelectedItem_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (!Convert.ToBoolean(hdnIsCash.Value))
            {
                ...
                lblT_price1.Text = nFintRetail.ToString("c");
            }
