...
public partial class CustomerSelectionInfoSec : System.Web.UI.Page
{
    ...
    protected void btnClearSignature_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (btnClearSignature.Text.Equals("Edit"))
            {
                DataClassesDataContext _db = new DataClassesDataContext();
        ...
        try
        {
            ...
            if (objCust != null)
            {
                ...
                if (imgSign != "data:image/jpeg;base64,")
                {
                    ...
                    btnClearSignature.Text = "Edit";
                    btnClearSignature.Visible = false;
                }
                else
                {
                    divSig1.Visible = true;
    ...
    private void SendApproveEmail(int nCustomerId)
    {
        try
        {
            ...
            string strBCCEmail = ConfigurationManager.AppSettings["DevBCC"].ToString();
            if (strToEmail.Length > 4)
            {
                csEmailAPI email = new csEmailAPI();
                customeruserinfo objCustUser = new customeruserinfo();
                ...
                {
                    objCustUser = (customeruserinfo)Session["oCustomerUser"];
                    strUser = objCustUser.customerusername;
                    int spid = Convert.ToInt32(hdnSalesPersonId.Value);
                ...
                email.Subject = "Customer " + objCust.last_name1 + " approved selection(s)";
                email.Body = strTable;
