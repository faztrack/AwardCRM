...
public partial class leadlist : System.Web.UI.Page
{
    ...
    protected void GetCustomersNew(int nPageNo)
    {
        ...
        if (txtSearch.Text.Trim() != "")
        {
            ...
            else if (ddlSearchBy.SelectedValue == "3")
            {
                ...
                strCondition = "  customers.email LIKE '%" + str.Replace("'", "''") + "%'";
            }
    ...
    protected void grdLeadList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            hyp_DocumentManagement.NavigateUrl = "DocumentManagement.aspx?cid=" + ncid + "&nbackId=2";
    ...
    private decimal GetRetailTotal(int EstID, int ncustid)
    {
        ...
        cus_est = _db.customer_estimates.Single(ce => ce.customer_id == ncustid && ce.client_id == Convert.ToInt32(ConfigurationManager.AppSettings["client_id"]) && ce.estimate_id == EstID);
        ...
        else
        {
            ...
            if (Convert.ToBoolean(!cus_est.IsCashTerm))
            {
                ...
                if (cus_est.FinancePer > 0)
                {
                    nFinanceAmountRetail = Convert.ToDecimal((dRetail * cus_est.FinancePer) / 100);
