...
public partial class selectionofsection : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString.Get("cid") != null && Request.QueryString.Get("eid") != null) // Customer Schedule
            {
                ...
                hypSelectionLog.NavigateUrl = "selectionlog.aspx?eid=" + nEstimateID + "&cid=" + nCustomerID;
                hypSelectionLog.Target = "_blank";
                ...
                if (_db.HistorySection_Selections.Any(se => se.customer_id == nCustomerID && se.estimate_id == nEstimateID))
                {
                    hypSelectionLog.Visible = true;
                }
                else
                {
                    hypSelectionLog.Visible = false;
                }
    ...
    private void GetSectionSelectionListData()
    {
        ...
        dtLocation = (DataTable)Session["ssLocation"];
        dtSection = (DataTable)Session["gSection"];
        var sslist = _db.Section_Selections.Where(ss => ss.customer_id == nCustId && ss.estimate_id == nEstId).ToList().OrderByDescending(c => c.CreateDate);
    ...
    protected void grdSelection_RowEditing(object sender, GridViewEditEventArgs e)
    {
        ...
        LinkButton btn = (LinkButton)grdSelection.Rows[e.NewEditIndex].Cells[8].Controls[0];
        btn.Text = "Update";
    ...
    protected void DeleteUploadedFile(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string deleteFolderPath = ConfigurationManager.AppSettings["UploadDir"] + "//" + hdnCustomerID.Value + "//DELETEDSELECTIONS//";
            string deletedFileThumbnailPath = ConfigurationManager.AppSettings["UploadDir"] + "//" + hdnCustomerID.Value + "//DELETEDSELECTIONS//Thumbnail//";

            if (Directory.Exists(deleteFolderPath) == false)
            {
                Directory.CreateDirectory(deleteFolderPath);

            }
            if (Directory.Exists(deletedFileThumbnailPath) == false)
            {
                Directory.CreateDirectory(deletedFileThumbnailPath);
            ...
            var upList = (from upl in _db.file_upload_infos
                          ...
                          select new
                          {
                              upload_fileId = upl.upload_fileId,
                              CustomerId = upl.CustomerId,
                              Desccription = upl.Desccription,
                              client_id = upl.client_id,
                              estimate_id = upl.estimate_id,
                              is_design = upl.is_design,
                              type = upl.type,
                              vendor_cost_id = upl.vendor_cost_id,
                              IsSiteProgress = upl.IsSiteProgress,
            ...
            if (Directory.Exists(sFolderPath))
            {
                ...
                {
                    Historyfile_upload_info objUH = new Historyfile_upload_info();

                    objUH.upload_fileId = file.upload_fileId;
                    objUH.CustomerId = file.CustomerId;
                    objUH.Desccription = file.Desccription;
                    objUH.client_id = file.client_id;
                    objUH.estimate_id = file.estimate_id;
                    objUH.is_design = file.is_design;
                    objUH.type = file.type;
                    objUH.vendor_cost_id = file.vendor_cost_id;
                    objUH.is_design = file.IsSiteProgress;
                    objUH.ImageName = file.ImageName;
                    if (Session["oUser"] != null)
                    {
                        userinfo objUser = (userinfo)Session["oUser"];
                        objUH.DeletedBy = objUser.first_name + " " + objUser.last_name;
                    }
                    else
                        objUH.DeletedBy = User.Identity.Name;

                    objUH.DeleteDate = DateTime.Now;
                    _db.Historyfile_upload_infos.InsertOnSubmit(objUH);
                    _db.SubmitChanges();


                    if (file.ImageName.Contains(".jpg") || file.ImageName.Contains(".jpeg") || file.ImageName.Contains(".png") || file.ImageName.Contains(".gif"))
                    {
                        string Thubmpath = Server.MapPath("File\\") + hdnCustomerID.Value + "\\SELECTIONS\\Thumbnail" + @"\" + file.ImageName;
                        File.Copy(Thubmpath, deletedFileThumbnailPath + file.ImageName);
                    }
                    
                    if (isSelected)
                    {
                        path = Server.MapPath("UploadedFiles\\") + hdnCustomerID.Value + "\\SELECTIONS" + @"\" + file.ImageName;
                        File.Copy(path, deleteFolderPath + file.ImageName);
                    ...
                    {
                         path = Server.MapPath("File\\") + hdnCustomerID.Value + "\\SELECTIONS" + @"\" + file.ImageName;
                        File.Copy(path, deleteFolderPath + file.ImageName);
    ...
    protected void DeleteSelection(object sender, EventArgs e)
    {
        try
        {
            ...
            Section_Selection objS = _db.Section_Selections.SingleOrDefault(s => s.SectionSelectionID == nSectionSelectionID);
            HistorySection_Selection objHS = new HistorySection_Selection();
            objHS.SectionSelectionID = objS.SectionSelectionID;
            objHS.customer_id = objS.customer_id;
            objHS.estimate_id = objS.estimate_id;
            objHS.section_id = objS.section_id;
            objHS.section_name = objS.section_name;
            objHS.location_name = objS.location_name;
            objHS.location_id = objS.location_id;
            objHS.Title = objS.Title;
            objHS.Price = objS.Price;
            objHS.Notes = objS.Notes;
            objHS.LastUpdateDate = objS.LastUpdateDate;
            objHS.UpdateBy = objS.UpdateBy;
            objHS.isSelected = objS.isSelected;
            objHS.customer_signature = objS.customer_signature;
            objHS.customer_signedBy = objS.customer_signedBy;
            objS.customer_siignatureDate = objS.customer_siignatureDate;
            objHS.CreateDate = objS.CreateDate;
            objHS.ValidTillDate = objS.ValidTillDate;
            objHS.CreatedBy = objS.CreatedBy;
            objHS.UserEmail = objS.UserEmail;
            if (Session["oUser"] != null)
            {
                userinfo objUser = (userinfo)Session["oUser"];
                objHS.DeleteBy = objUser.first_name + " " + objUser.last_name;
            }
            else
                objHS.DeleteBy = User.Identity.Name;

            objHS.DeletedDate = DateTime.Now;
            _db.HistorySection_Selections.InsertOnSubmit(objHS);
            _db.SubmitChanges();
            ...
            if (objS.isSelected == true)
            {
                if (Directory.Exists(sFolderPath))
                {
                    ...
                    foreach (var file in upList)
                    {
                        ...
                        //Copy Deleted file
                        string path = Server.MapPath("UploadedFiles\\") + hdnCustomerID.Value + "\\SELECTIONS" + @"\" + file.ImageName;
                        File.Copy(path, deleteFolderPath + file.ImageName);
            ...
            else
            {
                if (Directory.Exists(sFolderPath))
                {
                    ...
                    foreach (var file in upList)
                    {
                        ...
                        //Copy Deleted file
                        string path = Server.MapPath("File\\") + hdnCustomerID.Value + "\\SELECTIONS" + @"\" + file.ImageName;
                        File.Copy(path, deleteFolderPath + file.ImageName);
    ...
    string CreateHtml()
    {
        ...
        foreach (GridViewRow di in grdSelection.Rows)
        {
            ...
            if (chkSelected.Checked && isSelected==false)
            {

                string strQ = "SELECT * FROM Section_Selection WHERE  isSelected = 0 AND SectionSelectionID=" + nSectionSelectionID + " AND customer_id =" + Convert.ToInt32(hdnCustomerID.Value) + " AND estimate_id =" + Convert.ToInt32(hdnEstimateID.Value) + "order by CreateDate DESC";
                        ...
                        if (nDay >= 0)
                        {
                            ...
                            if (dtFile.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow drf in dtFile.Rows)
                                {
                                    ...
                                    string imgUrl = "https://awardkb.faztrack.com/";
