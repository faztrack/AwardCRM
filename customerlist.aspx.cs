...
public partial class customerlist : System.Web.UI.Page
{
    ...
    private void BindJobNumber()
    {
        try
        {
            ...
            string strQ = @"select ce.job_number as jobNumber
                           from customers as c
    ...
    {
        DataClassesDataContext _db = new DataClassesDataContext();
        string strQ = "select first_name+' '+last_name AS Superintendent_name,user_id from user_info WHERE user_id != 6 "; // role_id = 4";
        List<userinfo> mList = _db.ExecuteQuery<userinfo>(strQ, string.Empty).ToList();
    ...
    protected void GetCustomersNew(int nPageNo)
    {
        ...
        if (txtSearch.Text.Trim() != "")
        {
            ...
            {
                strCondition = "  ce.job_number LIKE '%" + str.Replace("'", "''") + "%'";
            }
        }
        else
        {
